...
//获取页面最大宽度
    var page_width = Math.max(maskElement.scrollWidth, window.innerWidth);

//获取页面最大高度
    var page_height = Math.max(maskElement.scrollHeight, window.innerHeight);
...
//水印数量自适应元素区域尺寸
    ...
    for (var i = 0; i < defaultSettings.watermark_rows; i++) {
        ...
        distance_y = defaultSettings.watermark_y + (defaultSettings.watermark_y_space + defaultSettings.watermark_height) * (i + 1);
        for (var j = 0; j < defaultSettings.watermark_cols; j++) {
            x = defaultSettings.watermark_x + (defaultSettings.watermark_width + defaultSettings.watermark_x_space) * j;
            distance_x = defaultSettings.watermark_x + (defaultSettings.watermark_width + defaultSettings.watermark_x_space) * (j + 1);
            if (distance_x >= window.innerWidth || distance_y >= window.innerHeight){
                break
            }
            // 高度或者宽度
