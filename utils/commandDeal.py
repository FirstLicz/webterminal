...
class CommandDeal(object):
    ...
    def remove_obstruct_char(cmd_str):
        ...
        # _cmd_str = control_char.sub('', cmd_str.strip())
        # print(f"cmd_str = {_cmd_str}")
        # if _cmd_str == cmd_str:
        #     head_char = re.compile(r'(\x1b]\S+?\x07\x1b\[\S+?)\[', re.X)
        #     # cmd_str = head_char.sub('', )
        # else:
        #     cmd_str = _cmd_str
...
if __name__ == '__main__':
    s = ['\x1b]0;root@localhost:~\x07\x1b[?1034h[root@localhost ~]# ', 'l', 'l']
    rett = CommandDeal().deal_command(''.join(s))
    print(rett)
